...
Say stuff
"""
import numpy, array, time, os, sys
...
input_binary = p.ssp_output
output_binary = p.synmag_output
input_head,input_tail = os.path.split(input_binary)

files = glob(p.filt_dir+p.filt_names) #Array of filter paths
files.sort()

tot = p.tot #Filter to use as total magnitude
mlr = p.mlr #Filter to use for mass-to-light ratio

z = numpy.arange(p.zmin,p.zmax,p.zstep)
z1= z+1

print('{0:s} {1:.1f} {2:s} {3:.1f} {4:s} {5} {6:s} {7}').format('Redshifts range from',p.zmin,'to',p.zmax,'in',len(z),'steps','\n') 

print('{0:s} {1:s}{2:s}').format('Loading input SEDs from -',input_tail,':')

with numpy.load(input_binary) as data:
    print('{0:30s}').format('Parameters'),
    parameters = data['parameters'][0]
    print('Done')

    print('{0:30s}').format('Wavelength Array'),
    wave = parameters[0]
    print('Done')

    print('{0:30s}').format('Age Array'),
    tg = parameters[1]
    print('Done')

    print('{0:30s}').format('SEDs'),
    SED = data['SED']
    print('Done')

    print('{0:30s}').format('Stellar Masses'),
    STR = data['STR']
    print('Done \n')

    print('{0:30s}').format('Star Formation Rates'),
    SFR = data['SFR']
    print('Done \n')
...
only
"""
print "Setting up redshift dependent arrays:"

#dl = [dm(z[i]) for i in range(len(z))]
#dl = numpy.array(dl)
print('{0:30s}').format('Ages'),
age = [tl(z[i]) for i in range(len(z))]
age = (t0()-numpy.array(age))*1e9
print('Done')

print('{0:30s}').format('Distance Moduli'),
...
print('{0:30s}').format('Madau Absorption Decrements'),
...
print('{0:40s} {1:15s} {2:15s} {3:20s}').format('Filter', 'Filter length','Prep Time', 'Calc Time (per z)') 

for filt in range(len(files)):
    head, tail = os.path.split(files[filt])
    print '{0:40s}'.format(tail),
    sys.stdout.flush()
    ...
    for zi in range(len(z)):
        ...
        for i in range(nwf):
            ...
            tpa = tpwf[i]*((1-a)*(SED[j,:ai[zi],:]*lyman_abs[j,zi]) + a*SED[j+1,:ai[zi],:]*lyman_abs[j+1,zi])
        ...
        F_mean[filt,zi,:ai[zi],:] = WR/2/z1[zi]/f_mean2/2.997925e18
...
if compute_MUV:
    ...
    for zi in range(len(z)):
        ...
        F_mean_UV[zi,:ai[zi],:] = WR/2/z1[zi]/f_mean2/2.997925e18
