...
class CSP:
    ...
    def build(self, age, sfh, dust, metal, fesc = 1.,
        ...
        for idT, t in enumerate(self.tau):
            ...
            self.weights = np.abs(self.sfr_func(self.tg[None, :, None] - self.ta_sfh,
                                         *tau)) / self.norm
...
class LoadEAZYFilters(object):
    ...
    def search(self, searchterm, error=0.1):
        if type(searchterm) == str:
            matches = np.array([searchterm in name for name in self.filternames])
            sorted_id = np.arange(len(self.filternames))[matches]
        elif type(searchterm) == u.quantity.Quantity:
            sorted_id = np.where(np.abs(self.central_wl - searchterm) < error*searchterm)[0]
        
        print('{0:<5s} {1:<10s} Angstrom       {2}'.format('Index',
                                       'Lambda_c',
                                       'Name'))
        for i in sorted_id:
            print('{0:<5.0f} {1:<8.2f} Angstrom      {2}'.format(i,
